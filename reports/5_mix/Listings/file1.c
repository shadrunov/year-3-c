...
int main()
{
    ...
    // char chFileName[] = {"AlekseyShadrunov.txt"};
    ...
    while ( 1 )
    {
        ...
        else if ( strncasecmp(command, "open", strlen(command)) == 0 )
        {
            ...
            ret = LabOpenFile(file_name, &hFile, &len);
        ...
        {
            // argument
            printf("enter handle argument: \n");
            HANDLE hFile;
            char hFile_str[10];
            fgets(hFile_str, 10, stdin);
            hFile = (void *)strtol(hFile_str, NULL, 10);
            printf("pointer: %d\n", hFile);

            // argument
            printf("enter file value: \n");
            char file_value[256];
            fgets(file_value, sizeof(file_value), stdin);
            file_value[strcspn(file_value, "\n")] = 0;
            printf("file value: %s\n", file_value);

            ret = LabWriteFile(hFile, file_value, &len);
        }
